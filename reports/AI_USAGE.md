...
During development, AI models occasionally produced incorrect or incomplete outputs. Below are detailed examples of these errors, how they were identified, and how they were corrected.
...
### Example 6: Infinite Re-rendering Loop in React Balances View
- **AI Suggestion**: Gemini suggested using a React `useEffect` hook to calculate net group balances and update component state on rendering. However, it incorrectly included the state setters themselves as dependencies:
  ```javascript
  useEffect(() => {
...
- **Correction**: The calculation logic was refactored to run as a computed value during render time via `useMemo`, eliminating the state synchronization hook entirely:
  ```javascript
...
### Example 7: Faulty Member Removal Handler
- **AI Suggestion**: Gemini generated a membership deletion router that fetched and deleted records based solely on the `user_id`:
  ```python
...
- **Correction**: Refactored the endpoint signature to accept both `group_id` and `user_id`, ensuring deletions are isolated to the targeted membership relation:
  ```python
...
### Example 8: CSV Date Comparison Type Mismatch
- **AI Suggestion**: In the CSV import anomaly detection module, Gemini suggested comparing CSV date columns directly against member joined/left dates:
  ```python
...
- **Correction**: Vectorized date parsing was applied using Pandas to convert string columns to datetime dates before running validation loops:
  ```python
